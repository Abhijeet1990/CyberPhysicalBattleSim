...
class PhysicalAttack(VulnerabilityOutcome):

    def __init__(self):
        # solve the power flow and get the results
        env = GridEnv()
        env.phy_env.SolvePowerFlow()
        voltages = env.phy_env.get_power_flow_results('bus').loc[:,'BusPUVolt'].astype(float)
        voltlist = voltages.values.tolist()
        self.voltages = voltlist
...
class NodeInfo:
    ...
    # Bool indicating if the node can cause phy contingency
    affect_phy: bool = False
...
class Identifiers(NamedTuple):
    ...
    # Array of all possible physical compromise
    phy_vulnerabilities: List[VulnerabilityID] = []
...
def infer_constants_from_nodes(
    ...
    return Identifiers(
        ...
        phy_vulnerabilities=collect_vulnerability_ids_from_nodes_bytype(
            nodes, vulnerabilities, VulnerabilityType.EXECUTE)
    )

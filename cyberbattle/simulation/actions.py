...
class AgentActions:
    ...
    def __process_outcome(self,
        ...
        # add the outcome conversion for model that is a Physical Attack type
        elif isinstance(outcome, model.PhysicalAttack):
            outcome.voltages = voltages
            for voltage in outcome.voltages:
                reward += (1.0 - voltage)**2
    ...
    def perform_phy_attack(self, node_index, value, env) -> ActionResult:
        command_list = []
        command_list.append([node_index+1,'1',value])
        output = env.ChangeParametersMultipleElement('gen', ['BusNum', 'GenID', 'GenVoltSet'], command_list)
        env.SolvePowerFlow()
        new_gen_data = env.get_power_flow_results('bus').loc[:,'BusPUVolt'].astype(float)

        # transform to the original values
        vulnerability_id = 'VoltageSetPoint'
        # this line beneath need to change if the order of the nodes alters
        target_node_id = ''
        if node_index+1 == 1:
            target_node_id='sA_dnp'
        elif node_index+1 == 2:
            target_node_id = 'sB_dnp'
        elif node_index+1 == 3:
            target_node_id = 'sC_dnp'

        target_node_info = self._environment.get_node(target_node_id)
        succeeded, result = self.__process_outcome(
            model.VulnerabilityType.EXECUTE,
            vulnerability_id,
            target_node_id,
            target_node_info,
            local_or_remote=False,
            ...
            # allow agent attempts to explore potential remote vulnerabilities
            throw_if_vulnerability_not_present=False,
            voltages=new_gen_data.values.tolist()
        )
        return result

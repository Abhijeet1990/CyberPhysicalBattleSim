...
from ..simulation import commandcontrol, cp_model, actions
...
Observation = TypedDict(
    'Observation', {
        ...
        'credential_cache': List[cp_model.CachedCredential],
        ...
        'discovered_nodes': List[cp_model.NodeID],
        ...
        # will add a physical observation space.. example may be voltage level of all the nodes in the sys.. think later
...
class EnvironmentBounds(NamedTuple):
    """Define global bounds posisibly shared by a set of CyberPhysicalBattle gym environments
    ...
    phy_count             - Unique phy vulnerabilities.. attack on breaker/gen set point/etc.
    """
    ...
    def of_identifiers(cls,
                       identifiers: cp_model.Identifiers,
                       maximum_total_credentials: int,
                       maximum_node_count: int,
        ...
        return EnvironmentBounds(
            ...
            phy_count=len(identifiers.phy_vulnerabilities),
            scan_count=maximum_node_count,
            patch_count=maximum_node_count
        )
...
# creation of the fresh new gym environment of the new cyber-physical battle
class CyPhyBattleEnv(gym.Env):
    ...
    # Actions
        ...
        Execute physical attack:       `(source_node x physical_attack)'
    ...
    The simulation ends if either the attacker reaches its goal (e.g. full network ownership and also change phy parameter),
    ...
    @property
    def environment(self) -> cp_model.Environment:
        return self.__environment

    def __reset_environment(self) -> None:
        self.__environment: cp_model.Environment = copy.deepcopy(self.__initial_environment)
        self.__discovered_nodes: List[cp_model.NodeID] = []
    ...
    @property
    def name(self) -> str:
        return "CyPhyBattleEnv"

    @property
    def identifiers(self) -> cp_model.Identifiers:
    ...
    def validate_environment(self, environment: cp_model.Environment):
        """Validate that the size of the network and associated constants fits within
        the dimensions bounds set for the CyberPhysicalBattle gym environment"""
        ...
        assert environment.identifiers.phy_vulnerabilities
        ...
        effective_maximum_credentials_per_action = max([
            ...
            if isinstance(vulnerability.outcome, cp_model.LeakedCredentials)])
        ...
        referenced_ports = cp_model.collect_ports_from_environment(environment)
        ...
        referenced_properties = cp_model.collect_properties_from_nodes(cp_model.iterate_network_nodes(environment.network))
        ...
        local_vulnerabilities = \
            cp_model.collect_vulnerability_ids_from_nodes_bytype(
                environment.nodes(),
                environment.vulnerability_library,
                cp_model.VulnerabilityType.LOCAL
            )
        ...
        remote_vulnerabilities = \
            cp_model.collect_vulnerability_ids_from_nodes_bytype(
                environment.nodes(),
                environment.vulnerability_library,
                cp_model.VulnerabilityType.REMOTE
            )
        ...
        phy_vulnerabilities = \
            cp_model.collect_vulnerability_ids_from_nodes_bytype(
                environment.nodes(),
                environment.vulnerability_library,
                cp_model.VulnerabilityType.EXECUTE
            )

        undefined_phy_vuln = set(phy_vulnerabilities).difference(environment.identifiers.phy_vulnerabilities)
        if undefined_phy_vuln:
            raise ValueError(f"The network has references to undefined physical"
                             f" vulnerability names: {undefined_phy_vuln}")

    # number of distinct privilege levels
    privilege_levels = cp_model.PrivilegeLevel.MAXIMUM + 1
    ...
    def __init__(self,
        ...
        self.__initial_environment: cp_model.Environment = initial_environment
        ...
        self.observation_space = spaces.Dict({
            ...
            # Esclation result
            'escalation': spaces.MultiDiscrete(cp_model.PrivilegeLevel.MAXIMUM + 1),
            ...
            'nodes_privilegelevel': spaces.MultiDiscrete([CyPhyBattleEnv.privilege_levels] * maximum_node_count),
            ...
            # internal IDs of the credentials in the cache
            'credential_cache': DummySpace(sample=[cp_model.CachedCredential('Sharepoint', "HTTPS", "ADPrincipalCreds")]),
    ...
    def __index_to_local_vulnerabilityid(self, vulnerability_index: int) -> cp_model.VulnerabilityID:
    ...
    def __index_to_remote_vulnerabilityid(self, vulnerability_index: int) -> cp_model.VulnerabilityID:
    ...
    def __index_to_physical_vulnerabilityid(self, vulnerability_index: int) -> cp_model.VulnerabilityID:
        """Return the remote vulnerability identifier from its internal encoding index"""
        return self.__initial_environment.identifiers.phy_vulnerabilities[vulnerability_index]

    def __index_to_port_name(self, port_index: int) -> cp_model.PortName:
    ...
    def __internal_node_id_from_external_node_index(self, node_external_index: int) -> cp_model.NodeID:
    ...
    def __find_external_index(self, node_id: cp_model.NodeID) -> int:
        """Find the external index associated with the specified node ID"""
        return self.__discovered_nodes.index(node_id)

    def __agent_owns_node(self, node_id: cp_model.NodeID) -> bool:
    ...
    def __update_action_mask(self, bitmask: ActionMask) -> None:
        ...
        for source_node_id in self.__discovered_nodes:
            if self.__agent_owns_node(source_node_id):
                ...
                # Physical: if that physical node is discovered to be compromised
                count = 0
                for node_id, node_data in self.__environment.nodes():
                    # if the discovered node has physical impact
                    if node_id in self.__discovered_nodes and node_data.affect_phy:
    ...
    def __execute_action(self, action: Action) -> actions.ActionResult:
        ...
        if "local_vulnerability" in action:
            ...
            result = self._actuator.perform_phy_attack(target_node,0.0, self.env.phy_env)
    ...
    def __property_vector(self, node_id: cp_model.NodeID, node_info: cp_model.NodeInfo) -> numpy.ndarray:
    ...
    def __observation_reward_from_action_result(self, result: actions.ActionResult) -> Tuple[Observation, float]:
        ...
        if isinstance(outcome, cp_model.LeakedNodesId):
        ...
        elif isinstance(outcome, cp_model.LeakedCredentials):
            # update discovered nodes and credentials
            newly_discovered_nodes_count = 0
            newly_discovered_creds: List[Tuple[int, cp_model.CachedCredential]] = []
        ...
        elif isinstance(outcome, cp_model.LateralMove):
            obs['lateral_move'] = numpy.int32(1)
        elif isinstance(outcome, cp_model.CustomerData):
            obs['customer_data_found'] = numpy.int32(1)
        elif isinstance(outcome, cp_model.ProbeSucceeded):
            obs['probe_result'] = numpy.int32(2)
        elif isinstance(outcome, cp_model.ProbeFailed):
            obs['probe_result'] = numpy.int32(1)
        elif isinstance(outcome, cp_model.PrivilegeEscalation):
            obs['escalation'] = numpy.int32(outcome.level)
        # update the next physical state
        elif isinstance(outcome, cp_model.PhysicalAttack):
            obs['voltage_levels'] = outcome.voltages
    ...
    def sample_action_in_range(self, kinds: Optional[List[int]] = None) -> Action:
        ...
        if kind == 2:
            ...
            action = Action(execute=numpy.array([np_random.randint(3)], numpy.int32))
    ...
    def is_action_valid(self, action, action_mask: Optional[ActionMask] = None) -> bool:
        ...
        if kind == "local_vulnerability":
            ...
            target_node = action["execute"]
            in_range = target_node < self.__bounds.phy_count
    ...
    def __get_explored_network(self) -> networkx.DiGraph:
        ...
        for node_id in subgraph.nodes:
            node_info: Optional[cp_model.NodeInfo] = subgraph.nodes[node_id]['data']
